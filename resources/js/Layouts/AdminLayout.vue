...
const showFlashMessage = computed(() => flash.value && (flash.value.success || flash.value.error));
...
const navItems = computed(() => {
    ...
    return [
        ...
        { 
            ...
            subitems: [
                { 
                    label: language.educational_subjects || 'Educational Subjects', 
                    href: '/admin/educational-subjects' 
                },
                { 
                    label: language.manage_educational_support || 'Manage Educational Support', 
                    href: '/admin/educational-support' 
                },
                {
                    label: language.educational_support_payments || 'Educational Support Payments',
                    href: '/admin/educational-support-payments'
                }
            ]
        },
        { 
            label: language.formations || 'Formations', 
            key: 'formations',
            hasSubmenu: true,
            subitems: [
                { label: (language.formations || 'Formations') + ' Subjects', href: '/admin/formations' },
                { label: (language.subscriptions || 'Subscriptions'), href: '/admin/formation-subscriptions' },
                { label: (language.payments || 'Payments'), href: '/admin/formation-payments' },
            ]
        },
        { 
            label: language.childhood_education || 'Childhood Education', 
            key: 'childhood_education',
            hasSubmenu: true,
            subitems: [
                { label: language.childhood_subjects || 'Childhood Subjects', href: '/admin/childhood-subjects' },
                { label: language.childhood_subscriptions || 'Childhood Subscriptions', href: '/admin/childhood-subscriptions' },
        ...
        { 
            label: language.expenses || 'Les dépenses', 
            key: 'expenses',
            hasSubmenu: true,
            subitems: [
                { label: language.expense_list || 'Liste des dépenses', href: '/admin/expenses' },
                { label: language.expense_categories || 'Catégories de dépenses', href: '/admin/expense-categories' },
            ]
        },
...
<template>
    <div class="flex min-h-screen flex-col bg-white">
        ...
        <div class="flex flex-1 relative">
                        ...
                        <template v-for="item in navItems" :key="'side-'+(item.href || item.key)">
                            ...
                            <InertiaLink v-if="!item.hasSubmenu" :href="item.href"
                                ...
                                <div class="w-5 h-5 flex-shrink-0">
                                    ...
                                    <!-- Expenses Icon -->
                                    <svg v-else-if="item.href === '/admin/expenses'" class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                ...
                                >
                                    <div class="flex items-center gap-3">
                                        ...
                                        <div class="w-5 h-5 flex-shrink-0">
                                            ...
                                            <!-- Formations Icon -->
                                            <svg v-else-if="item.key === 'formations'" class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 14l9-5-9-5-9 5 9 5z"></path>
                                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 14l6.16-3.422a12.083 12.083 0 01.665 6.479A11.952 11.952 0 0012 20.055a11.952 11.952 0 00-6.824-2.998 12.078 12.078 0 01-.665-6.479L12 14z"></path>
                                            </svg>
                                            <!-- Childhood Education Icon -->
                                            <svg v-else-if="item.key === 'childhood_education'" class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 6.253v13m0-13C10.832 5.477 9.246 5 7.5 5S4.168 5.477 3 6.253v13C4.168 18.477 5.754 18 7.5 18s3.332.477 4.5 1.253m0-13C13.168 5.477 14.754 5 16.5 5c1.747 0 3.332.477 4.5 1.253v13C19.832 18.477 18.246 18 16.5 18c-1.746 0-3.332.477-4.5 1.253"></path>
                                            </svg>
                                            <!-- Reports Icon -->
                                            <svg v-else-if="item.key === 'reports'" class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 17v-2m3 2v-4m3 4v-6m2 10H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z"></path>
                                            </svg>
                                            <!-- Expenses Icon -->
                                            <svg v-else-if="item.key === 'expenses'" class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 8c-1.657 0-3 .895-3 2s1.343 2 3 2 3 .895 3 2-1.343 2-3 2m0-8c1.11 0 2.08.402 2.599 1M12 8V7m0 1v8m0 0v1m0-1c-1.11 0-2.08-.402-2.599-1M21 12a9 9 0 11-18 0 9 9 0 0118 0z"></path>
                                            </svg>
                                        </div>
                                        <span class="font-medium">{{ item.label }}</span>
            ...
            >
                <!-- Flash Messages -->
                <div v-if="showFlashMessage" class="mb-6">
                    <div 
                        v-if="flash.success"
