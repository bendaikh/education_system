...
const totalClasses = ref(props.total || 50);
const classesPerPage = ref(4);

// Pagination
const totalPages = Math.ceil(totalClasses.value / classesPerPage.value);
const startIndex = (currentPage.value - 1) * classesPerPage.value + 1;
const endIndex = Math.min(currentPage.value * classesPerPage.value, totalClasses.value);
...
<template>
  <AdminLayout :title="language.classes || 'Classes'">
    <Head :title="language.classes || 'Classes'" />
        ...
        <div class="flex-1">
          <div class="relative">
            ...
            <input
              v-model="searchQuery"
              type="text"
              :placeholder="(language.search || 'Search') + ' ' + (language.classes || 'classes') + '...'"
    ...
    <div class="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
      ...
      <div class="hidden md:block overflow-x-auto">
        <table class="w-full">
          <thead class="bg-gray-50 border-b border-gray-100">
            <tr>
              <th class="text-left py-4 px-6 text-xs font-medium text-gray-500 uppercase tracking-wider">{{ language.class_name || 'Class Name' }}</th>
              <th class="text-left py-4 px-6 text-xs font-medium text-gray-500 uppercase tracking-wider">{{ language.teacher_name || 'Teacher' }}</th>
              <th class="text-left py-4 px-6 text-xs font-medium text-gray-500 uppercase tracking-wider">{{ language.students_count || 'Students' }}</th>
              <th class="text-left py-4 px-6 text-xs font-medium text-gray-500 uppercase tracking-wider">{{ language.schedule || 'Schedule' }}</th>
          ...
          <tbody class="divide-y divide-gray-100">
            <tr v-for="classItem in classes" :key="classItem.id" class="hover:bg-gray-50 transition-colors duration-150">
              <td class="py-4 px-6">
                <div class="text-sm font-medium text-gray-900">{{ classItem.name }}</div>
              </td>
              <td class="py-4 px-6">
                <div class="text-sm text-gray-600">{{ classItem.teacher }}</div>
              </td>
              <td class="py-4 px-6">
                <div class="text-sm text-gray-600">{{ classItem.students }}</div>
              </td>
              <td class="py-4 px-6">
                <div class="text-sm text-gray-600">{{ classItem.schedule }}</div>
              </td>
              <td class="py-4 px-6">
      ...
      <div class="md:hidden divide-y divide-gray-100">
        <div v-for="classItem in classes" :key="classItem.id" class="p-4 sm:p-6">
          <div class="flex items-start justify-between mb-3">
            <div class="flex-1 min-w-0">
              <h3 class="text-base font-medium text-gray-900 truncate">{{ classItem.name }}</h3>
          ...
          <div class="grid grid-cols-2 gap-4 text-sm">
            <div>
              <span class="text-gray-500">{{ (language.students_count || 'Students') + ':' }}</span>
              <span class="ml-1 text-gray-900">{{ classItem.students }}</span>
            </div>
            <div>
              <span class="text-gray-500">{{ (language.schedule || 'Schedule') + ':' }}</span>
              <span class="ml-1 text-gray-900">{{ classItem.schedule }}</span>
            </div>
          </div>
        </div>
      </div>
      ...
      <div class="px-4 sm:px-6 py-4 border-t border-gray-100 bg-gray-50">
        <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
          <div class="text-sm text-gray-700">
            Showing <span class="font-medium">{{ startIndex }}</span> to <span class="font-medium">{{ endIndex }}</span> of <span class="font-medium">{{ totalClasses }}</span> {{ language.classes || 'Classes' }}
          </div>
