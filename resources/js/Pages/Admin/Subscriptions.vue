...
const totalSubscriptions = ref(props.total || 42);
const subscriptionsPerPage = ref(4);

// Pagination
const totalPages = Math.ceil(totalSubscriptions.value / subscriptionsPerPage.value);
const startIndex = (currentPage.value - 1) * subscriptionsPerPage.value + 1;
const endIndex = Math.min(currentPage.value * subscriptionsPerPage.value, totalSubscriptions.value);
          ...
          <tbody class="divide-y divide-gray-100">
            <tr v-for="subscription in subscriptions" :key="subscription.id" class="hover:bg-gray-50 transition-colors duration-150">
              <td class="py-4 px-6">
                ...
                <div class="text-xs text-gray-500 mt-1">{{ subscription.features }}</div>
              </td>
              <td class="py-4 px-6">
                <div class="text-sm text-gray-600">{{ subscription.subscriber_name }}</div>
              </td>
              <td class="py-4 px-6">
      ...
      <div class="lg:hidden divide-y divide-gray-100">
        <div v-for="subscription in subscriptions" :key="subscription.id" class="p-4 sm:p-6">
          <div class="flex items-start justify-between mb-3">
            ...
            <span :class="getStatusClass(subscription.status)" class="px-2 py-1 text-xs font-medium rounded-full ml-3">
              {{ subscription.status }}
            </span>
          </div>
