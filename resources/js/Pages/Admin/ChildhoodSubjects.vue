<template>
    <AdminLayout :title="language.childhood_subjects || 'Childhood Subjects'">
        <div class="space-y-6">
            <!-- Header -->
            <div class="flex justify-between items-center">
                <h1 class="text-2xl font-bold text-gray-900">{{ language.childhood_subjects || 'Childhood Subjects' }}</h1>
                <button
                        ...
                        <tbody class="bg-white divide-y divide-gray-200">
                            <tr v-for="subject in subjects" :key="subject.id" class="hover:bg-gray-50">
                                ...
                                <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                                    {{ subject.duration === 'monthly' ? (language.monthly || 'Monthly') : 
                                       subject.duration === 'yearly' ? (language.yearly || 'Yearly') : 
                                       subject.duration || '-' }}
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                                ...
                                <div class="w-full">
                                    <div class="flex items-center justify-between mb-4">
                                        <h3 class="text-lg leading-6 font-medium text-gray-900" id="modal-title">
                                            {{ showEditModal ? (language.edit || 'Edit') : (language.add_new || 'Add New') }} {{ language.childhood_subjects || 'Childhood Subject' }}
                                        </h3>
                                    ...
                                    <form @submit.prevent="showEditModal ? updateSubject() : createSubject()" class="space-y-4">
                                        ...
                                        <div>
                                            <label class="block text-sm font-medium text-gray-700 mb-1">
                                                {{ language.name || 'Name' }} <span class="text-red-500">*</span>
                                            </label>
                                        ...
                                        <div>
                                            <label class="block text-sm font-medium text-gray-700 mb-1">
                                                {{ language.duration || 'Duration' }} <span class="text-red-500">*</span>
                                            </label>
                                            <select v-model="form.duration" required
                                                   class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent">
                                                <option value="">{{ language.select_duration || 'Select Duration' }}</option>
                                                <option value="monthly">{{ language.monthly || 'Monthly' }}</option>
                                                <option value="yearly">{{ language.yearly || 'Yearly' }}</option>
                                            </select>
                                        </div>
                                        ...
                                        <div class="grid grid-cols-2 gap-4">
                                            <div>
                                                ...
                                                <input v-model.number="form.school_percent" type="number" step="0.01" min="0" max="100" required
                                                       class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent"
                                                       placeholder="50.00" />
                                            </div>
                                            <div>
                                                ...
                                                <input v-model.number="form.teacher_percent" type="number" step="0.01" min="0" max="100" required
                                                       class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent"
                                                       placeholder="50.00" />
                                            </div>
                                        </div>
                                        ...
                                        <div>
                                            ...
                                            <div class="relative">
                                                ...
                                                <div class="min-h-[40px] border border-gray-300 rounded-md p-2 bg-white">
...
const createSubject = () => {
    ...
    router.post('/admin/childhood-subjects', {
        name: form.name,
        description: form.description,
        duration: form.duration,
        price: form.price,
        school_percent: form.school_percent,
        teacher_percent: form.teacher_percent,
        status: form.status,
        teachers: teacherIds
    }, {
        onSuccess: () => {
...
const updateSubject = () => {
    ...
    router.put(`/admin/childhood-subjects/${editingSubject.value.id}`, {
        name: form.name,
        description: form.description,
        duration: form.duration,
        price: form.price,
...
const deleteSubject = (subject) => {
    if (confirm('Are you sure you want to delete this subject?')) {
        router.delete(`/admin/childhood-subjects/${subject.id}`);
    }
};
</script>
